...
import {
  ...
  User, BedDouble, AlertTriangle, Loader2, Sparkles, ChevronDown, ChevronUp,
} from "lucide-react";
...
export default function DischargePage() {
  ...
  const [aiSummary,    setAiSummary]    = useState<Record<string, string> | null>(null);
  ...
  async function generateAiSummary() {
    if (!admission?.encounter_id) return;
    setAiLoading(true);
    setAiError(null);
    try {
      const res = await fetch("/api/ai/discharge-summary", {
        ...
        body: JSON.stringify({ encounterId: admission.encounter_id }),
      });
  ...
  return (
    <>
      ...
      <main className="p-6 max-w-2xl mx-auto space-y-5">
        ...
        {/* AI Discharge Summary */}
        <Card style={{ borderColor: BORDER, background: SURFACE }}>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2 text-base">
                <Sparkles size={17} color={PRIMARY} /> AI Discharge Summary
              </CardTitle>
              <Button
                size="sm"
                onClick={aiSummary ? () => setSummaryOpen(!summaryOpen) : generateAiSummary}
                disabled={aiLoading}
                style={{ background: `${PRIMARY}20`, color: PRIMARY }}
                className="gap-1.5 text-xs font-bold border-0 hover:opacity-80"
              >
                {aiLoading
                  ? <><Loader2 size={13} className="animate-spin" /> Generating…</>
                  : aiSummary
                    ? <>{summaryOpen ? <ChevronUp size={13} /> : <ChevronDown size={13} />} {summaryOpen ? "Collapse" : "Show"}</>
                    : <><Sparkles size={13} /> Generate with Claude</>
                }
              </Button>
            </div>
          </CardHeader>
          ...
          {aiSummary && summaryOpen && (
            <CardContent className="pt-2 space-y-3">
              {[
                ...
                { key: "hospitalCourse",         label: "Hospital Course" },
                { key: "proceduresDone",         label: "Procedures" },
                { key: "labHighlights",          label: "Lab Highlights" },
                { key: "medicationsOnDischarge", label: "Medications on Discharge" },
                ...
                <div key={key} className="rounded-lg border p-3" style={{ borderColor: BORDER }}>
                  <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: PRIMARY }}>{label}</p>
                  <p className="text-xs text-gray-300 leading-relaxed">{aiSummary[key] ?? "Not documented"}</p>
                </div>
              ))}
              <p className="text-[10px] text-gray-600">
                Generated by Claude claude-sonnet-4-6 from EMR notes — verify before signing.
              </p>
            </CardContent>
          )}
        </Card>
